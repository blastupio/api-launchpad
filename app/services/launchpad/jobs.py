...
from app.models import LaunchpadContractEventType, TransactionPaymentMethod, OperationType
from app.schema import CreateLaunchpadEvent, CreateLaunchpadTransactionParams
from app.services.launchpad.redis_cli import launchpad_events_cache
...
class MonitorLaunchpadLogsAndSave(Command):
    ...
    async def command(
        ...
        try:
            ...
            await self._save_user_registered_events(crud, user_registered_events)
    ...
    async def _save_user_registered_events(self, crud: LaunchpadContractEventsCrud, events):
        for event in events:
...
class SaveLaunchpadTransactionAndAddPoints(Command):
    ...
    async def command(
        ...
        points_amount = PointsCalculator.calculate_points_for_ido_purchase(token_amount_usd)
