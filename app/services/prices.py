...
from app.services.coingecko.client import coingecko_client
...
async def get_tokens_price(chain_id: ChainId, token_addresses: list[str]) -> dict[Address, float]:
    ...
    coingecko_id_by_chain_id_and_address = (
        await coingecko_client.get_coingecko_id_by_chain_id_and_address()
    )
    token_address_by_coingecko_id = coingecko_client.get_token_address_by_coingecko_id(
    ...
    coingecko_prices: dict[CoinGeckoCoinId, dict[str, float]] = (
        await coingecko_client.get_coingecko_price(token_ids)
    )
