...
async def get_projects_ids_of_user(
    user_address: str, contract_project_id_by_project_id: dict[str, int]
) -> list[str]:
    ...
    project_ids_of_user = []
    for (success, data), project_id in zip(output_data, contract_project_id_by_project_id.keys()):
        ...
        if info.registered:
            project_ids_of_user.append(project_id)
